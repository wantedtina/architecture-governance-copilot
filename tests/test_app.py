...
def _review_inputs_app() -> AppTest:
    ...
    assert [item.value for item in app.header] == ["Stage 2 — Review Inputs"]
...
def _analyzed_app() -> AppTest:
    ...
    assert [item.value for item in app.header] == ["Stage 3 — Human Review"]
...
def _assert_active_step(app: AppTest, label: str) -> None:
    ...
    assert stepper_markup.count('<div class="agc-step ') == 4
...
def test_initial_page_has_required_controls_and_no_generated_outputs() -> None:
    ...
    assert [item.value for item in app.header] == ["Stage 1 — Draft Solution Intent"]
...
def test_drafted_si_can_be_confirmed_and_handed_to_existing_review_stage() -> None:
    ...
    assert app.button(key="agc_generate_si_draft").disabled
    _assert_active_step(app, "Draft Solution Intent")
    # AppTest does not persist a routed page selection between later
    # interactions, so explicitly select the drafting page before each action.
    app.switch_page("pages/solution_intent_drafting.py").run()

    app.button(key="agc_load_drafting_context").click().run()
    app.switch_page("pages/solution_intent_drafting.py").run()
...
def test_sample_load_and_analysis_show_draft_without_automatic_outputs() -> None:
    ...
    assert all("Stage 4" not in item.value for item in app.header)
...
def test_human_edit_and_exclusion_generate_reviewed_outputs() -> None:
    ...
    assert not app.exception
    assert [item.value for item in app.header] == ["Stage 4 — Generated Outputs"]
...
def test_invalid_review_date_shows_error_without_stale_outputs() -> None:
    ...
    assert all(item.value != "Stage 4 — Generated Outputs" for item in app.header)


def test_changed_inputs_make_analysis_stale_and_hide_previous_outputs() -> None:
    app = _analyzed_app()
    app.button(key="agc_confirm_review").click().run()
    assert any(item.value == "Stage 4 — Generated Outputs" for item in app.header)

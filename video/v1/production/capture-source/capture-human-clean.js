...
const click=async(l,label)=>{await cue(l,label);await clear();await l.click();await pause(650);};
...
await pause(1800);await cue(owner,'Edit owner');await clear();await owner.fill('Taylor Kim');await owner.press('Tab');await pause(250);await focus(owner);await pause(2200);
await owner.evaluate(el=>el.scrollIntoView({block:'start'}));await pause(2200);
await click(page.getByRole('tab',{name:/Findings ·/}),'Review findings');
const title=page.getByRole('textbox',{name:'Title',exact:true}).nth(2);await focus(title);await pause(900);
const include=page.getByRole('checkbox',{name:'Include in reviewed record',exact:true}).nth(2);await cue(include,'Exclude this finding');await clear();await include.focus();await include.press('Space');await pause(250);await focus(title);await pause(2200);
await click(button('Confirm Reviewed Record & Generate Outputs'),'Confirm reviewed record');await pause(1600);
}